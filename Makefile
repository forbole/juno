...
LD_FLAGS = -X github.com/saifullah619/juno/v3/cmd.Version=$(VERSION) \
	-X github.com/saifullah619/juno/v3/cmd.Commit=$(COMMIT)
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' -not -path "./venv" | xargs goimports -w -local github.com/saifullah619/juno
.PHONY: format
