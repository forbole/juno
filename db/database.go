...
type Database interface {
	...
	SaveBlock(b *tmctypes.ResultBlock, txs []types.Tx) error

	// SaveTx will be called to save each transaction contained inside a block.
	// NOTE. For each message inside msgs, SaveMsg will be called as well.
...
func SaveBlock(database Database, block *tmctypes.ResultBlock, txs []types.Tx) error {
	// Handle the block itself
	if err := database.SaveBlock(block, txs); err != nil {
